...
From this output, we can conclude that increasing the buffer size can slightly improve simulation time because it reduces the frequency of producers waiting when the buffer is full. However, the effect of buffer size is balanced by the number of consumers: adding more consumers usually has a bigger effect on making the simulation run faster because they can handle items at the same time. There is also some connection between the number of producers and the buffer size, but the main thing that affects how long the simulation takes is how many consumers there are and how quickly they can keep up with the producers. The fastest runs were `7` (`p=10`,`c=2`, `b=3`) and `16` (`p=10`,`c=2`, `b=10`). This suggests that having more producers and fewer consumers can sometimes make the simulation run faster, most likely because the work gets done quickly without too many threads switching back and forth. Overall, the results also show that the program correctly tracks aggregate sales across all runs, confirming the implementation works as expected.
